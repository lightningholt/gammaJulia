...
function gammaTrackedLinApprox(N, rcpt_types, fs, vv, c, J0, i2e)
    ...
    r_star =  kk*max([sum(vv[1:2:end]), sum(vv[2:2:end])], zeros(2)).^nn
    SpectE = zeros(length(fs), cons)
    #SpectI = zeros(length(fs), cons)
    ...
    for cc in 1:cons
        ...
        if rcpt_types == 1
            ...
            J = [(1-nmdaRatio) * r_star[1] * W[:,1] zeros(N) nmdaRatio * r_star[1] * W[:,1] zeros(N, 2) r_star[2] * W[:,2]; (1-nmdaRatio) * r_star[1] * W[:,1] zeros(N) nmdaRatio * r_star[1] * W[:,1] zeros(N, 2) r_star[2] * W[:,2]; (1-nmdaRatio) * r_star[1] * W[:,1] zeros(N) nmdaRatio * r_star[1] * W[:,1] zeros(N, 2) r_star[2] * W[:,2] ]
            #J = -I + repeat(J, 1, rcpt_types)
            Jacob = Diagonal(kron(inv.(tauS), ones(N)))*J.data
            JacobLambs, Jacobvec = eigen(Jacob) #eigenvalues and vectors of Jacob
